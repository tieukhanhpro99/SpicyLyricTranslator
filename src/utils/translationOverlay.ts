...
const JAPANESE_FINAL_PARTICLES = [
    '\u304b\u306a',
    '\u3088\u306d',
    '\u3060\u3088',
    '\u3060\u306d',
...
function updateWordSyncStates(doc: Document): void {
    ...
    doc.querySelectorAll('.slt-sync-translation').forEach((transLine) => {
        ...
        const originalLine = lineIndex >= 0
            ? getLineByElementIndex(getLyricLines(doc), lineIndex) || findOriginalLineForTranslation(transLineEl)
            : findOriginalLineForTranslation(transLineEl);
...
function createQualityIndicator(doc: Document, index: number): HTMLElement | null {
    ...
    label.textContent = isCached ? `Cached Â· ${apiLabel}` : `Fresh Â· ${apiLabel}`;
