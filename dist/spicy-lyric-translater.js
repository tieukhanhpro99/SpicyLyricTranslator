...
${text}`;
  ...
  function unpackSpicyLyricsPayload(packed) {
    try {
      if (!Array.isArray(packed) || packed.length !== 2)
        return null;
      const valuesRaw = packed[0];
      const streamRaw = packed[1];
      if (!Array.isArray(valuesRaw) || !Array.isArray(streamRaw))
        return null;
      if (valuesRaw.length > 1 << 22 || streamRaw.length > 1 << 24)
        return null;
      ...
      const readStream = () => {
        if (cursor >= stream.length)
          throw new Error("Unexpected end of packed lyrics stream");
      ...
      const decode = (depth) => {
        if (depth > 512)
          throw new Error("Packed lyrics depth limit exceeded");
        const op = readStream();
        ...
        switch (op) {
          case -1: {
            ...
            const obj = {};
            for (let i = 0; i < keyCount; i++)
              safeSet(obj, keys[i], decode(depth + 1));
            return obj;
          }
          case -2: {
            ...
            for (let i = 0; i < length; i++)
              arr[i] = decode(depth + 1);
            return arr;
          }
          case -3: {
            const length = readCount(1 << 20);
            const keyCount = readCount(1 << 16);
            if (length * keyCount > 1 << 22)
              throw new Error("Packed lyrics schema budget exceeded");
            const keys = new Array(keyCount);
            for (let i = 0; i < keyCount; i++)
              keys[i] = readKey();
            const arr = new Array(length);
  ...
  function normalizeCapturedLyricsData(data) {
  ...
  function computeRenderSignature(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const lineIndex = getLineElementIndex(lines[i], i);
  ...
  function renderedOverlayIndexes(doc) {
  ...
  function hasRenderedOverlayForCurrentLines(doc, lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const wants = currentConfig.mode === "replace" ? !!translation && translation !== originalText && !!line.parentNode : (!!translation || isBreak) && translation !== originalText && !!line.parentNode;
      if (wants)
        expectedIndexes.add(lineIndex);
    }
  ...
  function renderSignatureUnchanged(doc, lines) {
    ...
    if (lastRenderSigMap.get(doc) === sig && hasRenderedOverlayForCurrentLines(doc, lines))
      return true;
    ...
    lines.forEach((line, index) => {
      ...
      const refreshOriginal = () => {
        if (showInjectedOriginal) {
          if (existingOrig) {
            existingOrig.dataset.lineIndex = lineIndex.toString();
            existingOrig.dataset.forLine = lineIndex.toString();
  ...
  function updateWordSyncStates(doc) {
    ...
    doc.querySelectorAll(".slt-sync-translation").forEach((transLine) => {
      ...
      const lineIndex = parseInt(transLineEl.dataset.lineIndex || "-1", 10);
      const originalLine = lineIndex >= 0 ? getLineByElementIndex(getLyricLines(doc), lineIndex) || findOriginalLineForTranslation(transLineEl) : findOriginalLineForTranslation(transLineEl);
      if (!originalLine)
        return;
  ...
  function createQualityIndicator(doc, index) {
    ...
    label.textContent = isCached ? `Cached \xC2\xB7 ${apiLabel}` : `Fresh \xC2\xB7 ${apiLabel}`;
...
<div class="sl-modal-overlay sl-modal-overlay-animated" style="z-index: 100;">
  ...
  function parseNonNegativeIndex2(value) {
    if (!value)
      return null;
    const parsed = Number.parseInt(value, 10);
    return Number.isInteger(parsed) && parsed >= 0 ? parsed : null;
    ...
    try {
      ...
      const buildIndexMapsForLines = (targetLines) => {
        ...
        targetArr.forEach((line, domIdx) => {
          ...
          if (translation)
            translationsByIdx.set(lineIndex, translation);
          ...
          if (meta)
            qualityByIdx.set(lineIndex, meta);
          ...
          if (rom)
            romanizationByIdx.set(lineIndex, rom);
          ...
          if (orig)
            originalByIdx.set(lineIndex, orig);
        });
